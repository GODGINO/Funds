...
-   **基准持仓**: 表格的**最后一行**会显示一个特殊的“**基准持仓**”切片。它代表了您在录入初始持仓后、进行任何后续交易（买/卖）之前的投资组合状态。所有历史切片都可以与此基准进行比较，以评估后续操作的长期效果。该行会以**特殊的背景色和粗体字**进行高亮显示，以示区分。
...
| :--- | :--- | :--- | :--- |
| **切片日期** | `snapshotDate` | 发生交易的日期，或为“基准持仓” | `2023-08-15` |
...
3.  **核心计算逻辑 (`App.tsx`)**:
    ...
    -   **钩子内部逻辑**:
        a. 遍历 `processedFunds`，收集所有基金 `tradingRecords` 中的唯一日期，并排序，生成**历史切片**。
        b. 单独计算一个**基准切片**，该切片仅基于用户的初始持仓 (`userPosition`)，不应用任何 `tradingRecords`。
        c. 将历史切片（最新在前）和基准切片（在最后）组合成最终的数组。
