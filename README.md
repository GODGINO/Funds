...
- **[UI 优化] 隐私遮罩图表顶格显示**:
    ...
    - **视觉细节微调**:
        ...
        - **参考线极致区分**: 
            - **日间分割线**: 在 **Mode 0** 中使用 **实线 (Solid)**，作为日期维度的强分隔符。
            - **盘中标记线**: 在所有模式中（如 09:30/11:30 标记线）使用 **虚线 (Dashed)**，作为背景辅助线退居二线，突出实时数据折线。
            - **颜色统一**: 所有垂直参考线（实线/虚线）均使用相同的颜色值，确保视觉和谐。
    ...
    - **Zigzag 骨架线**: 
        ...
        - Mode 1 (今日分时) 包含 **0.1%** 阈值 Zigzag 趋势骨架线。渲染逻辑：线宽统一为 **1px**，且指数背景线覆盖于骨架线之**上**。
    - **Mode 2 动态窗口**:
