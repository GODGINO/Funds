...
- **持仓管理**:
    ...
    - **数据导入/导出与 Gist 同步**: 
        ...
        - **[新功能] GitHub Gist 同步**: 集成了 GitHub Gist 功能，实现多设备间的数据云同步。
            ...
            - **拉取 (Pull)**: 本地存有 Token 时，点击导入弹窗中的下载图标，可从指定的 Gist 地址拉取最新的持仓数据。
            - **同步 (Push)**: 本地存有 Token 时，点击导入弹窗中的上传图标，可将当前的持仓数据上传并覆盖到远程 Gist。
            - **按钮显示**: Gist 同步的**拉取**和**推送**按钮仅在本地存储中有 Token 时才会显示。
