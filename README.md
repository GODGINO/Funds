...
- **持仓管理**:
    ...
    - **数据导入/导出**: 在添加基金表单旁提供“导入”功能。在弹出的窗口中，**输入框会默认显示您当前所有的持仓数据**，方便您**备份、复制或进行微调**。您也可以清空输入框，然后粘贴符合特定格式的新 JSON 字符串，或者**直接上传一个 JSON 文件**（应用会自动解析文件中的 `subscriptions` 数组），以批量替换或恢复所有持仓数据。此外，您还可以点击“**导出文件**”按钮，将当前的所有持仓数据保存为一个以当天日期命名的 JSON 文件（例如 `2024-07-26.json`），方便备份和迁移。应用会对数据格式进行校验，确保导入的安全性。导入过程采用**并行加载**，即使导入大量基金也能快速完成，并能跳过无效数据，确保最大成功率。**为了提升效率，当您通过‘上传文件’成功导入数据后，系统将自动为您执行保存操作，无需再次点击按钮。**
