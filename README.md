...
- **[UI 优化] 隐私遮罩图表顶格显示**:
    ...
    - **Zigzag 骨架线**: 
        - Mode 0 (历史对比) 包含 **0.15%** 阈值 Zigzag 趋势骨架线，并在每天的 **09:30** 和 **11:30** 位置增加灰色垂直虚线参考线。
        - Mode 1 (今日分时) 包含 **0.1%** 阈值 Zigzag 趋势骨架线，并在 09:30/11:30 设实线参考线。
    - **Mode 2 动态窗口**:
        - **盘中时段**: 起点动态设为 **当前时间-5分钟**（9:35前锁定9:30），终点锁定为 **15:00**，实现盘中高倍缩放观察。
        - **参考线**: 在 **11:30** 位置设有灰色垂直参考线，辅助判断午盘位置。
        - **视觉优化**: 移除了右侧成交额分布点容器的灰色背景，采用纯白色背景以保持界面整洁。
