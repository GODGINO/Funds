...
- **[新功能] 智能推荐评分**:
    - **双向策略支持**: 提供了两种截然不同的评分算法，辅助不同风格的投资者决策。
        - **左侧 (抄底) 策略**: 核心逻辑是“越跌越买”。在基金估值低（分位点低）、近期趋势下跌、今日下跌时给出高分，适合寻找“黄金坑”机会。
        - **右侧 (追涨) 策略**: 核心逻辑是“强者恒强”。在基金估值低（作为安全垫）、近期趋势上涨、今日上涨时给出高分，适合顺势交易。
    - **多维度加权算法**: 
        - **估值 (权重 50%)**: 基于历史净值分位点。无论哪种策略，便宜（低估值）永远是加分项。
        - **近期趋势 (权重 30%)**: 基于 Zigzag 算法识别的波段涨跌幅。
        - **今日表现 (权重 20%)**: 基于实时估值涨跌。
    - **可视化交互**:
        - **智能信号展示**: 无论当前选择何种排序方式，系统都会实时计算左侧和右侧策略的评分。
        - **去噪显示**: 列表第一列下方会自动展示**所有可执行的信号**（评分>=70的“强力买入”或评分<40的“减仓”）。对于评分在 40-70 之间的“持有/观望”状态，系统会自动隐藏，避免信息干扰，让您一眼聚焦机会。
        - **排序功能**: 您依然可以在控制栏中选择“推荐: 左侧”或“推荐: 右侧”来对基金列表进行整体排序，方便集中查看高分或低分基金。
- **[新优化] 界面交互体验升级**:
...
- **持仓切片分析 (Portfolio Snapshot Analysis)**: 在您进行**第一笔交易后**，主列表上方会自动出现一个强大的决策复盘工具，它只在有交易记录可供对比时才会显示。
    ...
    -   **交互式详情**: **双击表格中的任意一行**，会弹出一个窗口，详细展示构成该切片的具体操作。
