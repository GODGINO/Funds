...
- **持仓管理**:
    ...
    - **数据导入/导出与 Gist 同步**: 
        - **本地导入/导出**: 支持通过粘贴 JSON 字符串或上传/下载 JSON 文件来备份和恢复持仓数据。系统兼容新旧版本数据格式，并能自动识别。
        - **[新功能] GitHub Gist 同步**: 集成了 GitHub Gist 功能，实现多设备间的数据云同步。
            - **Token 配置**: 在导入弹窗中，您可以直接输入您的 **GitHub Personal Access Token**。Token 会自动保存在浏览器的本地存储 (Local Storage) 中，无需在代码中硬编码。
            - **拉取 (Pull)**: 输入 Token 后，点击输入框左侧的下载图标，可从指定的 Gist 地址拉取最新的持仓数据并填充到输入框中。
            - **同步 (Push)**: 输入 Token 后，点击输入框右侧的上传图标，可将当前的持仓数据上传并覆盖到远程 Gist。需确保输入的 Token 具有 Gist 写权限。
            - **按钮显示**: 为了保持界面简洁，Gist 同步的**拉取**和**推送**按钮仅在您输入了 Token 后才会显示。
