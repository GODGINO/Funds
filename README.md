...
Vite 构建时会自动将其复制到站点根目录，满足微信“将验证文件部署在网站根目录”的要求。
...
- **[UI 优化] 隐私遮罩图表逻辑**:
    - 针对三点收盘后的数据重复问题进行了优化。
    - **逻辑调整**: 下午三点后，系统将自动识别今日数据并停止单独渲染“实时”线条，统一采用 `localStorage` 中的 5 条历史曲线进行对比。
    - **显示上限**: 全局保持最多 5 条对比曲线（三点前为 4 历史 + 1 实时，三点后为 5 历史），视觉更加清爽。
- **[新列] 标签分析“近期操作”**:
