...
- **持仓管理**:
    ...
    - **数据导入/导出**: 在添加基金表单旁提供“导入”功能。在弹出的窗口中，**输入框会默认显示您当前所有的持仓数据**，方便您**备份、复制或进行微调**。您也可以清空输入框，然后粘贴符合特定格式的新 JSON 字符串，或者**直接上传一个 JSON 文件**，以批量替换或恢复所有持仓数据。应用会自动识别并解析两种主流格式：
        1. **新版统一格式**: 即通过本工具“导出文件”功能生成的、在 `tradingRecords` 数组中包含了**已确认和待处理交易**的持仓数组。
        2. **旧版兼容格式**: 一个包含 `subscriptions` 键的对象，或是不包含交易记录的持仓数组。
        
        应用会对数据格式进行校验，确保导入的安全性。导入过程采用**并行加载**，即使导入大量基金也能快速完成，并能跳过无效数据，确保最大成功率。**为了提升效率，当您通过‘上传文件’成功导入数据后，系统将自动为您执行保存操作，无需再次点击按钮。**
        
        **[新功能] 复制外部格式 JSON**: 为了方便与其他系统（如 Notion 或其他自定义工具）集成，在导入/导出弹窗中新增了一个按钮。点击后，会将当前持仓数据导出为一个包含基金名称、代码、份额、成本及原始排序索引的特定 JSON 数组格式，并复制到剪贴板。
...
- **便捷操作**:
    ...
    - **弹窗快速关闭**: 所有功能弹窗（包括详情、交易、导入等）均支持通过点击遮罩层空白处或按下 ESC 键快速关闭，界面更简洁。
- **标签系统与筛选**:
...
- **标签分析表格**: 在主列表上方新增一个功能强大的“标签分析”表格，提供对您投资组合的宏观洞察。该表格会自动汇总您所有**自定义标签**下的基金，并进行深度分析：
    ...
    -   **交互优化**: 为提升双击筛选的体验，表格内的文字已设置为不可选中状态，避免了误操作。同时优化了表格首列（固定列）的悬停高亮效果，确保视觉一致性。
