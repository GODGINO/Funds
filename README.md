
# 基金趋势可视化与持仓管理工具

一款交互式的 Web 应用，旨在帮助用户跟踪、可视化、比较多支投资基金的历史表现，并管理个人持仓信息。

## 核心功能

- **[UI 优化] 隐私遮罩图表顶格显示**:
    - **小恐龙喷火特效 (Instant Sync)**: 
        - **代码修复**: 彻底解决了 `Unexpected token '}'` 的语法报错，优化了组件逻辑结构。
        - **视觉改进**: 使用了用户提供的**精确锯齿路径**（727x208 规格），外观更具锋芒与动感。
        - **同步优化**: 移除了特效的条件挂载逻辑，使其在 DOM 中始终保持就绪状态。同时取消了容器的 `transition-opacity` 过渡动画。这一改进使得火焰特效在鼠标悬浮时能与折线图表**瞬间且绝对同步**地出现，消除了之前的微小延迟感。
        - **宽度精修**: 维持 130% 宽度与 220% 高度的比例，精准包裹折线图核心区域。
        - **技术实现**: 采用 `mix-blend-mode: difference` 配合纯白填充，对下方图表元素进行实时、无延迟的反色处理。
    - **暗黑模式适配 (最终版)**: 彻底修复了暗黑模式下折线图不可见的问题。采用“高对比度实色板”逻辑：在深色背景下，不再依赖半透明色，主线强制显示为 **纯白色 (#FFFFFF)**，历史数据线使用 **Slate (石板灰) 高亮梯次** 展示。
    - **指数级颜色衰减**: 隐私模式下的 **Mode 2 (历史成交额分析)**，颜色透明度遵循 **指数级衰减逻辑**。今日数据对比度最高，随着天数往回追溯，色彩呈指数级变淡，营造出清晰的视觉透视与重点。
    - **数据驱动的日期选择**: 
        - **盘前 (0:00-9:15)**: 展示最近 **5天** 完整的本地历史行情。
        - **盘中 (9:15-15:00)**: 展示最近 **4天** 历史记录 + **1天** 实时更新的波动数据。
        - **盘后 (15:00-24:00)**: 今日数据已自动持久化，展示包含今日在内的最近 **5天** 历史行情。
    - **视觉细节微调 (适配 75% 缩放)**:
        - 针对高缩放环境进行了线宽补偿：
            - 背景参考线与普通历史线宽度提升至 **1.33px** (原 1px)。
            - 核心实时数据增强线（Zigzag 骨架线、Mode 2 今日线）提升至 **2px** (原 1.5px)。
        - **折线图容器优化**: 为隐私模式下的三个模式图表增加了 **1px** 的内边距，确保在低缩放倍率下线条不会因为紧贴边缘而被 Recharts 渲染引擎截断。
        - 核心实时数据线增强:
            - **Mode 0**: Zigzag 趋势骨架线。
            - **Mode 1**: Zigzag 趋势骨架线 + **双轴成交额柱状图背景**。成交额柱状图颜色根据主题优化：浅色模式透明度调高至 **0.55**（更重/更易区分），暗黑模式调低至 **0.2**（更浅/减少干扰）。
            - **Mode 2**: 今日实时成交额曲线。
        - 调深了 **Mode 0 & 1** 中的指数背景线灰色（从 Slate 300 提升至 Slate 400），使其作为参考底纹更加清晰可辨。
        - **参考线极致统一**: 移除了垂直参考线的粗细区分，**所有模式下**的垂直标记线统一使用相同的色值。
    - **Mode 0 视觉降噪**: 历史对比模式下，所有指数折线统一使用 **深灰色辅助色** 背景显示，取消颜色衰减，以极简视觉突出 **Zigzag 趋势骨架线**。
    - **主题检测增强**: 结合 `MutationObserver` 与 `window.matchMedia`，实现系统级主题偏好与应用内暗黑模式切换的完美同步。
    - **时间轴对齐**: 实时上证指数从 **09:15** 竞价段开始记录数据；两市成交额从 **09:30** 开始累加。
    - **Zigzag 骨架线**: 
        - Mode 0 (历史对比) 包含 **0.15%** 阈值 Zigzag 趋势骨架线。
        - Mode 1 (今日分时) 包含 **0.1%** 阈值 Zigzag 趋势骨架线。
    - **Mode 2 动态窗口**:
        - **盘中时段**: 起点动态设为 **当前时间-5分钟**（9:35前锁定9:30），终点锁定为 **15:00**，实现盘中高倍缩放观察。
        - **参考线**: 在 **11:30** 位置设有垂直**虚线**参考线，辅助判断午盘位置。
        - **视觉优化**: 移除了右侧成交额分布点容器的背景色，使其完全融入主题；优化了成交额分布点的色彩梯次。
        - **收盘时段 (15:00后)**: 起点回归 **09:30**，展示 09:30-15:00 全天量能分布。
    - **极简展示**: 移除图表交互游标与实时数值展示，使图表作为纯粹的视觉背景与持仓概况页脚配合。
    - **调整 Mode 1 背景量能**: 引入独立隐藏 Y 轴，显示每分钟成交额柱状图.
    - **坐标轴优化**: 采用 `type="number"` 数值轴映射，彻底解决 Recharts 分类轴带来的边缘留白问题。
- **[收盘持久化] 完整行情存储**:
    - 每天 15:00 收盘后，系统将自动把包含 **09:15-15:00** 完整波动（约 250+ 数据点）的行情序列保存至 `localStorage`。
