...
- **净值分位点**: 自动计算最新净值在选定历史周期（如近100天）内的百分比位置，并以“分位点”的形式展示。该数值有助于判断当前估值是处于历史高位（如接近100%）还是低位（如接近0%）。同时，该指标也被添加为新的排序选项。
- **拐点单元格高亮**: 在数据表格中，所有代表趋势拐点的单元格都会以**灰色背景**高亮显示，与图表上的“骨架”趋势线转折点完全对应，方便您快速定位到每一次趋势变化的关键节点。
- **集成的控制面板**: 在表格上方，提供了一个集中的控制区域，允许用户自由选择排序标准（如“近期趋势”、“今日涨幅”或“分位点”）、排序方式（升序或降序）、查看的历史记录数量以及调整趋势线的灵敏度。
...
3.  **分析对比**: 应用将获取数据并展示在表格中。您可以使用表格上方的控制面板，选择按“近期趋势”、“今日涨幅”或“分位点”进行排序，并随时切换升序或降序。
...
-   **数据处理与排序**: 应用使用 `useMemo` Hook 来派生出一个经过处理和排序的基金列表。它会为每支基金计算其近期趋势（自上一个拐点以来的涨跌幅）以及当前净值在历史区间内的分位点，然后根据用户选择的排序标准（如“近期趋势”、“今日涨幅”或“分位点”）对整个列表进行排序。这个经过排序的列表才是最终渲染到页面上的数据源，确保了性能和UI的一致性。
-   **数据持久化**:
