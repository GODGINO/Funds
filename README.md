...
- **[UI 优化] 精确滚动对齐**:
    - **滚动逻辑重构**: 优化了过滤和分析时的滚动行为。现在双击标签或长按持仓切片时，页面将通过 `offsetTop` 精确计算位置，确保基金列表卡片完美对齐到视口顶部，消除了之前的定位偏差。
- **[性能优化] 极速悬停联动**:
    - **持仓切片分析联动升级**: 为平衡操作便利性与流畅度，切片分析表格现已恢复为经典的“**悬停触发**”联动模式。通过**组件细粒度拆分**与 **React.memo** 策略，实现了行与图表的高频同步，彻底解决了之前版本中鼠标悬停时的卡顿感，交互反馈灵敏。
- **[UI 细节优化] 持仓切片分析表格**:
