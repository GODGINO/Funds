...
- **交互式交易日志 (Trading Journal)**:
    - **可视化交易历史**: 您的每一笔买卖操作都会在基金的**净值走势图**上以一个醒目的**红点 (买入)** 或**蓝点 (卖出)** 进行标记。将鼠标悬停在这些点上，可以立刻看到该笔交易的详细信息（如成交净值、金额/份额），让您的操作与市场波动之间的关系一目了然。
    - **上下文驱动的交易与编辑**:
        ...
        - 对于**待处理**的任务，点击“记录”按钮同样会打开一个编辑弹窗，您可以在此**修改**任务的金额/份额，或直接**取消**该任务。
    - **智能执行逻辑**:
        - **补录历史 (即时成交)**: 点击任意**历史日期**的买卖按钮，交易将**立即计算并生效**。
        - **当日交易 (自动处理)**: 点击**“当日净值”**列的买卖按钮，系统会自动判断：若净值**已确认**，则**立即成交**；若**未确认**，则为您创建一个**待处理的交易任务**，同时单元格状态会立即更新为“记录”并着色，防止重复操作。该任务将在净值更新后自动执行。
    - **健壮的数据一致性**:
        - 每当您修改或删除**任何一笔历史交易**，或在详情弹窗中更新您的**初始持仓**时，系统都会在后台启动一个“**顺序重算 (Replay)**”机制。它会从零开始，按时间顺序重新计算您的每一笔交易，以确保您当前的份额、平均成本和累计收益等所有财务数据都保持绝对的准确性。
    - **独立的交易弹窗**: “买入”和“卖出”拥有各自独立的、经过优化的弹窗界面，并内嵌了分析图表，支持您在交易前快速回顾近期表现和进行实时预估。
    - **交易任务管理**: 在底部的“交易管理”面板中，您可以轻松查看和取消所有**待处理**的交易任务。
- **持仓管理**:
    ...
    - **数据导入/导出**: 在添加基金表单旁提供“导入”功能。在弹出的窗口中，**输入框会默认显示您当前所有的持仓数据**，方便您**备份、复制或进行微调**。您也可以清空输入框，然后粘贴符合特定格式的新 JSON 字符串，或者**直接上传一个 JSON 文件**（应用会自动解析文件中的 `subscriptions` 数组），以批量替换或恢复所有持仓数据。应用会对数据格式进行校验，确保导入的安全性。导入过程采用**并行加载**，即使导入大量基金也能快速完成，并能跳过无效数据，确保最大成功率。
...
- **隐私模式**:
    ...
    - **信息速览与刷新**: 在遮罩状态下，您的今日预估总盈亏和收益率默认隐藏为`-`，当鼠标移动到遮罩上时才会显示具体数值，移开后再次隐藏。同时，还会显示实时的**上证指数**点位、涨跌额及涨跌幅（指数数据接口经过优化，稳定性更高），让您在隐藏个人信息的同时也能快速掌握大盘动态。单击鼠标左键会触发一次静默的数据刷新，并立即更新遮罩上显示的时间戳和盈亏数据。**全局的自动刷新（周一至周五交易时段内每三分钟一次）也会在后台照常运行**，并同步更新时间戳，确保遮罩下的数据也能保持最新。
    - **解除方式**: 在遮罩状态下，只需再次单击鼠标右键即可恢复显示。
    - **开关控制**: 在底部的基金添加表单区域，您可以随时通过“隐私模式”开关来启用或禁用此功能。
    - **禁用右键菜单**: 在隐私模式开启期间，浏览器的默认右键菜单会被屏蔽，以确保隐私模式的正常交互。
    - **文本防选中**: 遮罩层上的所有文字均已设置为不可选中状态，防止意外高亮。
- **移动端自动横屏**:
    - **优化体验**: 考虑到本应用包含大量数据表格和图表，在移动设备的竖屏模式下可能难以完整展示。为了提供最佳的浏览体验，应用引入了自动横屏功能。
    - **实现方式**: 当检测到浏览器窗口宽度小于 768px（典型的移动设备尺寸）时，整个页面会自动通过 CSS 旋转 90 度，强制以横屏模式进行布局。
    - **用户便利**: 用户无需手动旋转手机，即可获得为宽屏优化的完整视图，确保所有数据和图表都能清晰、完整地呈现。
- **便捷操作**:
...
- **标签分析表格**: 在主列表上方新增一个功能强大的“标签分析”表格，提供对您投资组合的宏观洞察。该表格会自动汇总您所有**自定义标签**下的基金，并进行深度分析：
    ...
    -   **交互优化**: 为提升双击筛选的体验，表格内的文字已设置为不可选中状态，避免了误操作。
...
- **持仓切片分析 (Portfolio Snapshot Analysis)**: 在您进行**第一笔交易后**，主列表下方会自动出现一个强大的决策复盘工具，它只在有交易记录可供对比时才会显示。
    - **基准持仓对比**: 表格的最后一行始终会显示您的“**基准持仓**”表现。它代表了忽略所有交易的**初始持仓**持有至今的表现，作为所有操作对比的起点。
    - **自动快照**: 每当您在任何一天进行交易操作（买入或卖出）时，系统都会自动为该日创建一个“持仓切片”——即您当日操作完成后的完整投资组合的精确快照。
    - **动态回溯评估**: 表格中的每一行代表一个历史切片。系统会用**今天最新**的基金净值，去重新评估这个历史持仓组合，计算出如果当初的持仓一直持有至今的各项表现指标：总成本、持有总值、累计总值、总收益、收益率、日收益及日收益率。
    ...
    - **交互式详情**: **点击表格中的任意一行**，会弹出一个窗口，详细展示构成该切片的具体操作。如果点击的是“基准持仓”，则会显示您的初始持仓列表；如果点击的是某个交易日，则会清晰地列出当日所有的买入和卖出记录。
...
- **交易回顾与即时反馈**: 在趋势信息下方，新增一行展示您最近一笔交易以来的表现，格式为 `X天前 [买入/卖出] [⬆︎/⬇︎]Y.YY% [👍/👎]`。
    - **智能评价**: 系统会自动为您评估这笔操作的“好坏”。👍 代表“正确”的操作（买入后上涨，卖出后下跌），👎 代表“错误”的操作（买入后下跌，卖出后上涨），为您提供即时的决策反馈。
- **趋势强度高亮**: 当一支基金的“近期趋势”涨跌幅绝对值超过 4.5% 时，其在主列表中的名称单元格会自动以浅灰色背景高亮显示，帮助您快速识别市场中出现显著变动的基金。
...
- **实时估值与自动刷新**: 应用会在**启动时**、**手动刷新时**以及**在周一至周五的交易时段（上午9点至下午4点）每 3 分钟自动刷新**时更新所有基金的实时估值，确保信息最新。所有刷新操作（手动或自动）都会同步更新隐私遮罩上的时间戳。
