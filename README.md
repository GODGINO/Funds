...
3.  **发起交易**: 在主数据表的任意日期下方，点击“买”或“卖”，在弹窗中输入金额或份额后确认。
4.  **修改/删除交易**: 如果某日已有交易，该日期的“买/卖”链接会变为“记录”按钮。点击它即可打开弹窗进行修改或删除。
5.  **分析对比**: 返回主列表，您可以看到所有基金的表现对比。使用表格上方的控制面板，选择按“近期趋势”、“今日涨幅”或“分位点”进行排序。
6.  **按标签筛选**: 点击主列表上方的任一标签，即可只查看该分类下的基金。
7.  **刷新估值**: 点击控制面板最右侧的刷新图标按钮，可以快速获取所有已跟踪基金的最新实时估值和您的持仓收益。
...
1.  **数据输入与持久化**:
    ...
    *   **持久化**: `App.tsx` 中的一个 `useEffect` 钩子会监听 `funds` 和 `tradingTasks` 状态的变化。一旦发生变化，它会将所有基金的 `userPosition` 数据和待处理任务提取出来，序列化为 JSON 字符串，并保存到浏览器的 `localStorage`。应用启动时，会反向执行此过程，从 `localStorage` 加载数据。
...
- **交易记录详情**: 提供一个界面，用于查看和管理单支基金的所有历史交易记录 (`TradingRecord`)。
