...
- **隐私模式**:
    ...
    - **信息速览与安全保护**: 在遮罩状态下，您的今日预估总盈亏和收益率默认隐藏为`-`，当鼠标移动到遮罩上时才会显示具体数值，移开后再次隐藏。
        ...
        - **[新功能] 联动老板键**: 在 PC 端，如果触发了上述 8秒 安全超时，系统不仅会隐藏数据，还会同时触发“老板键”，自动尝试唤起 **飞书 (Feishu)** 窗口，确保隐私绝对安全。
    - **实时动态**: 遮罩层还会显示实时的**上证指数**点位、涨跌额及涨跌幅，让您在隐藏个人信息的同时也能快速掌握大盘动态。单击鼠标左键会触发一次静默的数据刷新。**全局的自动刷新**也会在后台照常运行，确保遮罩下的数据保持最新。
